    ...
    def __init__(self, vocab_f, vocab_size):
        ...
        with open(vocab_f, 'r', encoding = 'utf8') as i:
            ...
            for l in lines:
                ...
                index += 1
        # 记录特殊符的索引
        self.padding_idx = self.token_idx_dict[PAD] # 填充
        self.unk_idx = self.token_idx_dict[UNK] # 未知单词
        self.eos_idx = self.token_idx_dict[EOS] # 句子结束符
...
class Data(torch.utils.data.Dataset):
    def __init__(self, train_path, dev_path, bert_vocab, seq_max_len, tgt_vocab_f, tgt_vocab_size):
        ...
        # vocab和tgt_vocab的区别是：
        # bert_vocab = BertTokenizer.from_pretrained(bert_path)(相当于source)
        # tgt_vocab = ./data/vocab.txt（是不是这个对应的是摘要的vocab）
        ...
        self.shuffle_train_idx() # 打乱训练集的索引
    ...
    def __len__(self):
        if self.mode == 'train':
            return len(self.train_article_list)
        else:
            return len(self.dev_article_list)

    def __getitem__(self, idx):
        if self.mode == 'train':
            curr_idx = self.train_idx_list[idx]
            one_inp_token_list = self.train_article_list[curr_idx]
            one_tgt_token_list = self.train_title_list[curr_idx]
        else:
            curr_idx = idx
            one_inp_token_list = self.dev_article_list[curr_idx]
            one_tgt_token_list = self.dev_title_list[curr_idx]

        tgt_valid_len = len(self.tgt_vocab.tokentoidx(one_tgt_token_list))
        if tgt_valid_len == 0:
            return self.__getitem__(np.random.randint(len(self)))

        if len(one_inp_token_list) <= tgt_valid_len:
            return self.__getitem__(np.random.randint(len(self)))

        if len(one_inp_token_list) >= self.seq_max_len or tgt_valid_len >= self.seq_max_len:
            return self.__getitem__(np.random.randint(len(self)))

        if len(one_inp_token_list) == 0 or len(one_tgt_token_list) == 0:
            return self.__getitem__(np.random.randint(len(self)))
        ...
        # assert len(one_inp_token_list) >= len(one_tgt_token_list) #涉及到的任务都是输入长于输出的，所以assert一下

        one_inp_token_list = self.vocab.convert_tokens_to_ids(one_inp_token_list)
        one_tgt_token_list = self.tgt_vocab.tokentoidx(one_tgt_token_list)

        return one_inp_token_list, one_tgt_token_list, one_ref_token_list

    def collate_fn(self, batch):
        batch_inp_list, batch_tgt_list, batch_ref_list = zip(*batch)

        max_inp_len = max([len(item) for item in batch_inp_list])
        # max_tgt_len = max([len(item) for item in batch_tgt_list])

        batch_inp_list = self.pad_data(batch_inp_list, max_inp_len, self.src_padding_idx)
        batch_tgt_list = self.pad_data(batch_tgt_list, max_inp_len, self.tgt_padding_idx)
        return batch_inp_list, batch_tgt_list, batch_ref_list
        ...
        if mode == 'train':
            if self.train_current_idx + batch_size < self.train_num - 1:
                for i in range(batch_size):
                    ...
                    # 先用curr_idx找到train_idx_list的idx，然后获取train_article_list的对应article
...
class DistributedDataset:
    def __init__(self, data, article_list, title_list):
        self.data = data
    ...
    def __getitem__(self, index):
        return self.article_list[index], self.title_list[index]

# class DDP_dataset(torch.utils.data.Dataset):
#     def __init__(self, train_path, dev_path, bert_vocab, seq_max_len, tgt_vocab_f, tgt_vocab_size):
#
